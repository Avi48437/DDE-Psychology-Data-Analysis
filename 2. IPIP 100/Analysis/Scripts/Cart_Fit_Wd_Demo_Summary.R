results_dir <- "/Users/avinandanroy/Desktop/Research/DDE/DDE_Fitting_Pshycology2/2. IPIP 100/Analysis/Results"
R <- readRDS(file.path(results_dir,"IPIP98_CART_Demographics_100.rds"))
...
tree_summary <- function(reps,model){
  ...
  if(length(vars)==0)
    return(data.frame(Variable=character(0),Root_n=numeric(0),Root_pct=numeric(0),
                      Used_n=numeric(0),Used_pct=numeric(0),Mean_splits=numeric(0)))
